...
$(document).ready(function() {
  ...
  var chartData = JSON.parse(cUser.getAttribute("data-acq-data"));
...
if (acquisition !== null) {
  // Retrieve the acqData from the data attribute
  var acqData = JSON.parse(acquisition.getAttribute("data-acq-data"));
...
if (activity !== null) {
  var activityCanvas = document.getElementById("activity");
  var activityData = JSON.parse(activityCanvas.getAttribute("data-activity-data"));

  var labels = activityData.map(function(data) {
    return data.date_label;
  });

  var activeUsersData = activityData.map(function(data) {
    return data.active_users;
  });

  var inactiveUsersData = activityData.map(function(data) {
    return data.inactive_users;
  });
  ...
  if (activity !== null) {
    ...
    items.forEach(function(item, index) {
      item.addEventListener("click", function() {
        config.data.datasets[0].data = activityData[index].active_users;
        config.data.datasets[1].data = activityData[index].inactive_users;
        myLine.update();
      });
    });
  }
